App = {
    ...
    bindEvents: function () {
        ...
        $('#fileUploader').on('change', this.upload);
    },
    ...
    upload: async function (e) {
        ...
        const uploadedFiles = await fetch(App.ipfsUrl + '/add', {method: "POST", body: formData})
            .then((res) => {
                if (res.ok) {
                    return res.text();
                }
            ...
            .then((txt) => {
                return txt.trim().split(/\s+/).map(txt => JSON.parse(txt));
            })
            .catch((err) => {
                console.error(err);
                alert(err);
        ...
        uploadedFiles.forEach(f => App.pictureHash = f.Hash);
