...
contract SupplyChain is Ownable, ConsumerRole, DistributorRole, FarmerRole, RetailerRole {
    ...
    checkValue(_upc)
    {
        ...
        address payable farmerAddress = payable(item.originFarmerID);
    ...
    )
    {
        ...
        // uint256,uint256,uint256,string memory,uint256,uint256,address,address,address)
        return
        (
        item.sku, // uint256
        item.upc, // uint256
        item.productID, // uint256
        item.productNotes, // string memory
        item.productPrice, // uint256
        uint(item.itemState), // enum SupplyChain.State
        item.distributorID, // ,address,address,address
        item.retailerID,
        item.consumerID
        );
        //        return
